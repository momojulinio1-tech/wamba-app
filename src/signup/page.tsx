    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        if (!name || !email || !password || !avatar) {
            toast({
                variant: "destructive",
                title: "Champs requis",
                description: "Veuillez remplir tous les champs, y compris l'avatar."
            });
            return;
        }
        setIsLoading(true);
        try {
            await createMemberByAdmin({ name, email, password, avatar });
            toast({
                title: "Inscription réussie !",
                description: "Votre compte est maintenant en attente d'approbation. Si vous êtes le premier utilisateur, vous serez administrateur.",
            });
            router.push('/login');
        } catch (error: any) {
             toast({
